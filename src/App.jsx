...
function makeCalcDoc(cust,b,c){
  ...
  const discItems=[["— Consumer Offer",c.cof?fc(c.cof):""],["— Special Discount",c.sdis?fc(c.sdis):""],["— Corporate Discount",c.corp?fc(c.corp):""],["— Dealer Discount"+(b.discRem?" ("+b.discRem+")":""),Number(b.discAmt||0)?fc(Number(b.discAmt)):""]].filter(([,v])=>v&&v!=="₹0");
  const items=[["(A) Ex-Showroom",fc(c.ex)],["+ Comp. Accessories",fc(c.ca)],["+ Handling",fc(c.hdl)],["+ Insurance (5yr)",fc(c.ins)],["+ Registration",fc(c.reg)],["+ Accessories",fc(c.acc)],["+ Teflon",fc(c.tef)],["+ Hypothication",fc(c.hyp)],["+ ATW",fc(c.atw)],["+ RSA",fc(c.rsa)],["+ AMC",fc(c.amcV)],["TOTAL ON-ROAD (C)",fc(c.C),"bold"],[""," "],...(discItems.length>0?[...discItems,...(discItems.length>1?[["Total Discount (D)",fc(c.D),"bold"]]:[["Discount (D)",fc(c.D),"bold"]])]:[]),["DEAL PRICE (E)",fc(c.E),"bold"],[""," "],["— Booking Amount",c.bk?fc(c.bk):""],["— Exchange Value",c.exv?fc(c.exv):""],["NET (G)",fc(c.G),"bold"],["— Loan / Disbursal",c.loan?fc(c.loan):""],["Balance from Customer (I)",fc(c.I),"bold"],...payItems,["DIFFERENCE (K)",fc(c.K),"bold"]];
...
function BillingModal({cust,onClose,onSave,onDraft,notify,role,stockData,billedChassis,allCusts}){
  ...
  function doSubmit(amtDiff){
    ...
    // Auto-send calc sheet to office on save
    try{
      const tempBilling={...f,details,calc:c,paid:c.paid,billModelCode,billModelName:RC[billModelCode]?.n||cust.model};
      const tempCust={...cust,...details,billing:tempBilling,billedDate:f.deliveryDate||td()};
      const calcDoc=makeCalcDoc(tempCust,tempBilling,c);
      const calcFname="CalcSheet_"+cust.name.replace(/ /g,"_")+"_"+td()+".pdf";
      const offNum=ld("nkd_office_wa",OFFICE_WA)||OFFICE_WA;
      sharePdf(calcDoc,calcFname,offNum,"Calculation Sheet for "+cust.name+" — "+(RC[billModelCode]?.n||cust.model),"Office");
      savePdfToDrive(calcDoc,calcFname,cust.name,"CalcSheet",cust.branch||"");
    }catch(ce){console.error("Calc sheet auto-send failed:",ce);}
  }
        ...
        <div style={{marginBottom:14}}>
          ...
          <div style={{background:"#ffffff",border:"1px solid #6b8fb5",borderRadius:12,padding:12}}>
            ...
            <div style={{display:"flex",alignItems:"center",padding:"4px 0",borderBottom:"1px solid #131820",gap:6}}>
              ...
              <input type="number" inputMode="decimal" value={f.discAmt||""} onChange={e=>setF(p=>({...p,discAmt:e.target.value}))} placeholder="0" style={{width:72,background:"#f1f5f9",border:"1px solid "+(Number(f.discAmt||0)<0?"#ef4444":"#6b8fb5"),borderRadius:8,padding:"5px 8px",fontSize:12,color:Number(f.discAmt||0)<0?"#ef4444":"#1e293b",textAlign:"right",flexShrink:0}}/>
              <button type="button" onClick={()=>setF(p=>({...p,discAmt:p.discAmt?String(-Number(p.discAmt)):""}))} style={{flexShrink:0,width:32,height:32,background:Number(f.discAmt||0)<0?"#fee2e2":"#f1f5f9",border:"1px solid "+(Number(f.discAmt||0)<0?"#ef4444":"#6b8fb5"),borderRadius:8,fontSize:14,fontWeight:700,color:Number(f.discAmt||0)<0?"#ef4444":"#475569",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>±</button>
            </div>
              ...
              {(f.payments||[]).map((p,i)=>{
                ...
                <div key={i} style={{marginBottom:8,background:locked?"rgba(100,116,139,0.06)":"rgba(249,115,22,0.04)",border:"1px solid "+(locked?"rgba(100,116,139,0.3)":"rgba(249,115,22,0.2)"),borderRadius:9,padding:"8px 10px",pointerEvents:locked?"none":"auto",opacity:locked?0.6:1}}>
                  ...
                  <div style={{display:"flex",gap:6,alignItems:"center"}}>
                    <input type="date" value={p.date||td()} onChange={e=>{const px=[...f.payments];px[i]={...px[i],date:e.target.value};setF(q=>({...q,payments:px}));}} style={{flex:1,background:"#f1f5f9",border:"1px solid #6b8fb5",borderRadius:8,padding:"5px 8px",fontSize:11,color:"#1e293b"}}/>
            ...
            <div style={{display:"flex",alignItems:"center",padding:"4px 0",borderBottom:"1px solid #131820",gap:6,marginTop:4}}>
              ...
              <input type="number" inputMode="decimal" value={f.excessAmt||""} onChange={e=>setF(p=>({...p,excessAmt:e.target.value}))} placeholder="0" style={{width:72,background:"#f1f5f9",border:"1px solid "+(Number(f.excessAmt||0)<0?"#ef4444":"#6b8fb5"),borderRadius:8,padding:"5px 8px",fontSize:12,color:Number(f.excessAmt||0)<0?"#ef4444":"#1e293b",textAlign:"right",flexShrink:0}}/>
              <button type="button" onClick={()=>setF(p=>({...p,excessAmt:p.excessAmt?String(-Number(p.excessAmt)):""}))} style={{flexShrink:0,width:32,height:32,background:Number(f.excessAmt||0)<0?"#fee2e2":"#f1f5f9",border:"1px solid "+(Number(f.excessAmt||0)<0?"#ef4444":"#6b8fb5"),borderRadius:8,fontSize:14,fontWeight:700,color:Number(f.excessAmt||0)<0?"#ef4444":"#475569",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>±</button>
            </div>
  ...
  return(
    <div>
      ...
      {<div style={{marginBottom:12}}>
        ...
        <button onClick={()=>{const doc=makeCalcDoc(cust,b,c);doc.save("CalcSheet_"+cust.name.replace(/ /g,"_")+"_"+td()+".pdf");}} style={{width:"100%",background:"rgba(99,102,241,0.1)",border:"1px solid rgba(99,102,241,0.35)",borderRadius:12,padding:13,color:"#6366f1",fontWeight:700,fontSize:13,cursor:"pointer",marginBottom:8}}>⬇️ Download Calc Sheet PDF</button>
        <div style={{display:"flex",gap:8}}>
...
function UserMgmt({nkdUsers,onSave,notify}){
  ...
  const [newSmBranch,setNewSmBranch]=useState(BRANCHES[0]);
  ...
  function addSM(){
    ...
    save({...users,salesman:[...(users.salesman||[]),{name:newSmName.trim(),phone,branch:newSmBranch}]});
    setNewSmName("");setNewSmPhone("");setNewSmBranch(BRANCHES[0]);notify("✅ Sales Executive added");
  }
  ...
  function updateSMBranch(name,branch){
    save({...users,salesman:(users.salesman||[]).map(u=>u.name===name?{...u,branch}:u)});
    notify("✅ Branch updated for "+name);
  }
    ...
    <div style={{maxWidth:720}}>
      ...
      <div style={{background:"#fff",border:"2px solid #22c55e",borderRadius:14,padding:"16px 18px",marginBottom:16}}>
        ...
        {(users.salesman||[]).map((u,i)=>(
          <SMRow key={i} u={u} onRemove={()=>removeSM(u.name)} onPhone={(p)=>updateSMPhone(u.name,p)} onBranch={(b)=>updateSMBranch(u.name,b)} onReset={()=>resetSMPass(u.name)} onUnlock={()=>unlockSM(u.name)}/>
        ))}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr auto",gap:8,marginTop:12,alignItems:"end",borderTop:"1px solid #e8eef8",paddingTop:12}}>
          ...
          <div><div style={{fontSize:11,color:"#64748b",fontWeight:600,marginBottom:4}}>Branch</div>
            <select style={{...inp,margin:0}} value={newSmBranch} onChange={e=>setNewSmBranch(e.target.value)}>
...
function SMRow({u,onRemove,onPhone,onBranch,onReset,onUnlock}){
  ...
  return(
    <div style={{display:"flex",alignItems:"center",gap:8,padding:"9px 0",borderBottom:"1px solid #e8eef8",flexWrap:"wrap"}}>
      ...
      {editingBranch?(
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          <select defaultValue={u.branch||""} onChange={e=>{onBranch(e.target.value);setEditingBranch(false);}} style={{...inp,margin:0,padding:"4px 8px",fontSize:12,minWidth:130}}>
            <option value="">-- Select --</option>
            {BRANCHES.map(b=><option key={b} value={b}>{b}</option>)}
          </select>
          <button onClick={()=>setEditingBranch(false)} style={{padding:"4px 8px",borderRadius:7,border:"1px solid #6b8fb5",background:"transparent",cursor:"pointer",fontSize:11,color:"#475569"}}>✕</button>
        </div>
      ):(
        <>
          <div style={{fontSize:11,color:u.branch?"#6366f1":"#ef4444",background:u.branch?"rgba(99,102,241,0.08)":"rgba(239,68,68,0.08)",borderRadius:6,padding:"2px 8px",fontWeight:600}}>
            {u.branch?"🏢 "+u.branch:"⚠️ No Branch"}
          </div>
          <button onClick={()=>setEditingBranch(true)} style={{padding:"4px 9px",borderRadius:7,border:"1px solid #6366f155",background:"rgba(99,102,241,0.05)",cursor:"pointer",fontSize:11,color:"#6366f1",fontWeight:600}}>
            {u.branch?"Move Branch":"Set Branch"}
          </button>
        </>
      )}
      {/* Phone */}
...
function CashBook({custs,smBranchMap}){
  const [date,setDate]=useState(td());
  const [branch,setBranch]=useState("All");
  // Collect all payments for the selected date
  const entries=[];
  custs.forEach(c=>{
    const br=c.branch||(smBranchMap&&smBranchMap[c.salesman])||SM_BRANCH[c.salesman]||"—";
...
function OwnerPortal({custs,stockData,billedChassis,statusData,role,user,mBr,saveStockData,saveStatusData,nkdUsers,onSaveUsers,notify,onUpd,onApprove,onLogout,onMobile}){
  ...
  const branches=BRANCHES.map(b=>({name:b,enq:custs.filter(c=>(c.branch||smBranchMap[c.salesman])===b).length,book:custs.filter(c=>(c.branch||smBranchMap[c.salesman])===b&&c.booking).length,bill:billed.filter(c=>(c.branch||smBranchMap[c.salesman])===b).length,rev:billed.filter(c=>(c.branch||smBranchMap[c.salesman])===b).reduce((s,c)=>s+((c.billing&&c.billing.calc&&c.billing.calc.E)||0),0)}));
