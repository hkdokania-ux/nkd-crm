  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:150,display:"flex",alignItems:"flex-end"}}>
      <div style={{background:"#ffffff",width:"100%",borderRadius:"20px 20px 0 0",maxHeight:"94vh",overflowY:"auto",padding:"20px 16px 44px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {[{k:"name",l:"Customer Name *",ac:"new-password",nm:"cust-fullname"},{k:"phone",l:"Phone *",t:"tel",ac:"new-password",nm:"cust-mobile"}].map(({k,l,t,ac,nm})=>(
            <div key={k}><label style={lbl}>{l}</label><input type={t||"text"} name={nm} autoComplete={ac} style={t!=="tel"?{...inp,textTransform:"uppercase"}:{...inp,borderColor:f[k]&&f[k].length!==10?"#ef4444":undefined}} value={f[k]||""} onChange={e=>{const v=t==="tel"?e.target.value.replace(/\D/g,"").slice(0,10):e.target.value;setF(p=>({...p,[k]:v}));}} {...(t!=="tel"?capBlur(k):{})}/>{t==="tel"&&f[k]&&f[k].length>0&&f[k].length!==10&&<div style={{fontSize:10,color:"#ef4444",marginTop:2}}>⚠️ Must be 10 digits ({f[k].length} entered)</div>}</div>
          ))}
          <div><label style={lbl}>Model — search by code or name</label>
            <input name="cust-model" autoComplete="new-password" style={{...inp,textTransform:"uppercase"}} value={mSearch} onChange={e=>{const v=e.target.value.toUpperCase();setMSearch(v);if(!v){pickM("");}}} placeholder="TYPE MODEL CODE OR NAME…" list="model-list"/>
          ...
          <div><label style={lbl}>Address</label><input name="cust-addr" autoComplete="new-password" style={{...inp,textTransform:"uppercase"}} value={f.address||""} onChange={e=>setF(p=>({...p,address:e.target.value}))} {...capBlur("address")}/></div>
        ...
        <div style={{marginBottom:14}}>
          ...
          <div style={{background:"#ffffff",border:"1px solid #6b8fb5",borderRadius:12,padding:12}}>
            ...
            <div style={{display:"flex",alignItems:"center",padding:"4px 0",borderBottom:"1px solid #131820",gap:6,marginTop:4}}>
              ...
              <input type="number" value={f.excessAmt||""} onChange={e=>setF(p=>({...p,excessAmt:e.target.value}))} placeholder="±0" style={{width:88,background:"#f1f5f9",border:"1px solid "+(Number(f.excessAmt||0)<0?"#ef4444":"#6b8fb5"),borderRadius:8,padding:"5px 8px",fontSize:12,color:Number(f.excessAmt||0)<0?"#ef4444":"#1e293b",textAlign:"right",flexShrink:0}}/>
            </div>
